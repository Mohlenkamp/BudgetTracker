...
const request = indexedDB.open('budgetTracker_IDB', 1);

// this event will emit if the database version changes (nonexistant to version 1, v1 to v2, etc.)
request.onupgradeneeded = function(event) {
    // save a reference to the database 
    const db = event.target.result;
    // create an object store (table) called `tally', set it to have an auto incrementing primary key of sorts 
...
request.onsuccess = function(event) {
    ...
    // check if app is online, if yes run uploadTally() function to send all local db data to api
    if (navigator.onLine) {
...
function saveRecord(record) {
    ...
    // access the object store for `tally`
    const tallyObjectStore = transaction.objectStore('tally');
  
    // add record to your store with add method
    tallyObjectStore.add(record);
  }
